...
def plot_predictions(df, symbol, forecast_30, forecast_60, forecast_90, save_chart=False, chart_path="stock_chart.png"):
...
if __name__ == "__main__":
    ...
    # Analyze using different indicator combinations
    for combination in range(1, 5):
        print(f"\nUsing Indicator Combination {combination}...")
        df_combined = calculate_indicators(df.copy(), combination)
        
        # Predict next day using the RandomForest model
        forecast_1 = predict_future(df_combined, 1)
        next_day_price = forecast_1[0]
        print(f"Predicted price for next day: {next_day_price:.2f}")

        # Predict 30, 60, 90 days using the RandomForest model
        ...
        # Generate Summary
        generate_summary(symbol, next_day_price, forecast_30, forecast_60, forecast_90, df_combined)

        # Plot Predictions and save the chart
        plot_predictions(df_combined, symbol, forecast_30, forecast_60, forecast_90, save_chart=True, chart_path=f"{symbol}_chart_combination_{combination}.png")
